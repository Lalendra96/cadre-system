...
@section(
    'title',
    'Service Letter — ' . $serviceLetter->subject
)
...
@php
    $user = auth()->user();
    $isApprover = $user->isAdministrativeOfficer() || $user->isSuperAdmin();
    $isOwner = $serviceLetter->drafted_by === $user->id;
    $canEdit = $serviceLetter->isEditable()
...
>
    ...
    >
        ...
        <div>
            ...
            >
                <span
                    class="sl-status sl-status-{{ $serviceLetter->status }}"
                >
                    {{
                        ucwords(
                            str_replace(
    ...
    >
        @if ($canEdit)
            <a
                href="{{ route('service-letters.edit', $serviceLetter) }}"
        ...
        <a
            href="{{ route('service-letters.print', $serviceLetter) }}"
            target="_blank"
...
>
    <div>
        <div class="sl-letter-body">
            {{ $serviceLetter->rendered_body }}
        </div>

        @if (
            $serviceLetter->status === 'approved'
            && $serviceLetter->eSignature
        )
            <div
                style="
                    margin-top: 16px;
            ...
            >
                ...
                <div>
                    ...
                    >
                        ...
                        {{ $serviceLetter->approved_at?->format('d M Y H:i') }}
                    </div>
                </div>
            </div>
        @endif
        ...
        @if ($serviceLetter->status === 'rejected')
            ...
            >
                ...
                <div class="md-body-sm">
                    {{ $serviceLetter->rejection_reason }}
                </div>
            </div>
        @endif
    </div>
    ...
    >
        ...
        >
            ...
            >
                ...
                <div>
                    ...
                    {{ $serviceLetter->employee->position->title ?? '—' }}
                </div>
                ...
                <div>
                    ...
                    {{ $serviceLetter->employee->subjectCode->code ?? '—' }}
                </div>
                ...
                <div>
                    ...
                    {{ strtoupper($serviceLetter->language ?? 'en') }}
                </div>
                ...
                <div>
                    ...
                    {{ $serviceLetter->template->name ?? 'Written from scratch' }}
                </div>
                ...
                <div>
                    ...
                    {{ $serviceLetter->updated_at->format('d M Y H:i') }}
                </div>
            </div>
        </div>
        ...
        @if (
            ...
            <form
                method="POST"
                action="{{ route('service-letters.submit', $serviceLetter) }}"
        ...
        @if (
            $isApprover
            && $serviceLetter->status === 'pending_approval'
        )
            <div
                class="md-card md-card--elevated"
                style="padding: 16px;"
            >
                ...
                <form
                    method="POST"
                    action="{{ route('service-letters.approve', $serviceLetter) }}"
                ...
                >
                    ...
                    >
                        ...
                        I confirm I am e-signing this letter as the
                ...
                <details>
                    ...
                    <form
                        method="POST"
                        action="{{ route('service-letters.reject', $serviceLetter) }}"
