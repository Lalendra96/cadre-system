...
class SubjectOfficerWorkspaceController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $currentEntry = CarderMonthlyEntry::whereIn(
            'subject_code_id',
            $codeIds
        )
            ->where('year', $now->year)
        ...
        $qualityIssues = app(
            DataQualityController::class
        )
            ->detectIssues($employees)
            ->count();
        ...
        $serviceLetters = ServiceLetter::whereIn(
            'employee_id',
            $employeeIds
        )
            ->where('is_active', true)
        ...
        $letterStats = [
            'draft' => $serviceLetters
                ->where(
                    'status',
                    ServiceLetter::STATUS_DRAFT
                )
                ->count(),
            'pending' => $serviceLetters
                ->where(
                    'status',
                    ServiceLetter::STATUS_PENDING_APPROVAL
                )
                ->count(),
            'rejected' => $serviceLetters
                ->where(
                    'status',
                    ServiceLetter::STATUS_REJECTED
                )
                ->count(),
        ];

        $pendingCorrections = EmployeeChangeRequest::whereIn(
            'employee_id',
            $employeeIds
        )
        ...
        $registrationsExpiring = $employees
            ->filter(
                fn ($employee) => $employee
        ...
        $gradePromotionReminders = $employees
            ->map(
                function ($employee) use ($now) {
            ...
            ->filter()
            ->sortBy(
                fn ($row) => $row->eligible_on
            );

        $gradePromotionEligibleNow = $gradePromotionReminders
            ->where('eligible_now', true)
            ->count();
        ...
        $incompleteServiceHistory = $employees
            ->filter(
                fn ($employee) => ! $employee
        ...
        $recentTransfers = TransferRecord::whereIn(
            'employee_id',
            $employeeIds
        )
            ->where('is_active', true)
